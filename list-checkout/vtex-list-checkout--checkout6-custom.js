/* source: <vtex.list-checkout@0.4.1> */
!(function (e) {
  var o = {};
  function a(n) {
  ...
  function (e, o) {
    function a() {
      $("textarea#cart-note").attr(
        "placeholder",
        "Deixe uma mensagem para o dono da lista..."
      ),
        $("textarea#cart-note").attr("maxlength", "280"),
        $("#count").length ||
          $("textarea#cart-note").after('<p id="count">0/280</p>'),
        $("textarea#cart-note").keyup(function () {
          $("#count").text(0 + $(this).val().length + "/280");
        });
    }
    $(document).ready(function () {
      ...
      $(window).on("orderFormUpdated.vtex", function (e, o) {
        console.log("orderForm BB", o),
          $("button#go-to-shipping").text("Salvar"),
          $("button#btn-go-to-payment").text("Salvar"),
          $(".link-note-add").text("Adicionar Mensagem"),
          $("#link-change-shipping").text("Alterar endereço de cobrança"),
          $("p.ship-receiverName").hide(),
          $("a#link-change-shipping").text("Alterar endereço de cobrança"),
          $("#open-shipping").text("Alterar endereço de cobrança"),
          $("a#force-shipping-fields").text("alterar"),
          $(
            "#shipping-data > div > div.accordion-inner.shipping-container > div > div.vtex-omnishipping-1-x-addressForm > div.vtex-omnishipping-1-x-address > div > p.input.ship-complement.text > label"
          ).html("Complemento"),
          $("#shipping-data .accordion-heading .accordion-toggle")
            .contents()
            .filter(function () {
              return "Entrega" === this.nodeValue;
            })
            .each(function () {
          ...
          $(
            'span[data-i18n="shipping.countrySelect.chooseDeliveryCountry"]'
          ).text("País"),
          $('span[data-i18n="shipping.addressForm.complement"]').text(
            "Complemento"
          ),
          $('label[data-i18n="global.note"]').text("Adicionar Mensagem");
      }),
      $(window).on("hashchange", () => {
        "#/profile" === window.location.hash &&
          ...
          "#/shipping" === window.location.hash &&
            ($('span[data-i18n="shipping.title"]').text("Endereço de Cobrança"),
            $(
              'span[data-i18n="shipping.countrySelect.chooseDeliveryCountry"]'
            ).text("País"),
            $('span[data-i18n="shipping.addressForm.complement"]').text(
            ...
            $('label[data-i18n="global.note"]').text("Adicionar Mensagem"),
            $("button#go-to-payment").text("Salvar"),
            $("#shipping-data .accordion-heading .accordion-toggle")
              .contents()
              .filter(function () {
                return "Complemento e referência" === this.nodeValue;
              })
              .each(function () {
                this.textContent = this.textContent.replace(
  ...
  function (e, o, a) {
    const { _locale: n } = a(0),
      { debounce: t, formatCurrency: r } = a(6),
      s = a(7);
    e.exports = class {
      constructor({
        type: e = "vertical",
        ...
        try {
          $.each(e.items, function (e) {
            ...
            $(".table.cart-items tbody tr.item-service").each(function (e) {
              if ($(this).find(".v-custom-trservice-wrap").length > 0)
                return !1;
              $(this)
                .find("> *")
      ...
      changeShippingTimeInfo() {
        ...
        $("body").addClass("v-custom-changeShippingTimeInfo"),
          $(".srp-delivery-current-many__sla").removeClass(
            "v-changeShippingTimeInfo-elem-active"
          );
        let o = [
        ...
        try {
          $(
            ...
            let a =
              ...
              n =
                vtexjs.checkout.orderForm.shippingData.logisticsInfo[0]
                  .selectedSla,
              t = a.find((e) => e.name == n).shippingEstimate,
              r = $(this)
                .find(
                  o
                    .map(
                      (e) => e + ":not(.v-changeShippingTimeInfo-elem-active)"
                    )
                    .join(", ")
                )
                .text();
            if ("" != r && r.match(/(day)|(dia)|(día)/gm)) {
              let a;
              if (
                ((a = $(this).hasClass("srp-delivery-current-many")
                  ? parseInt(t.match(/\d+/))
                  : parseInt(r.match(/\d+/))),
                a)
              ) {
          ...
          try {
            $.each(e.items, function (o) {
              ...
              if (
                ...
                return;
              let n = a.find(".total-selling-price:eq(0)").text(),
      ...
      customAddressFormInit(e) {
        let o = e || vtexjs.checkout.orderForm;
    ...
    e.exports = class {
      ...
      setForm(
        ...
        $(".vcustom--vtex-omnishipping-1-x-address #v-custom-ship-street").val(
          ...
          $(
            ".vcustom--vtex-omnishipping-1-x-address #ship-state option[value='" +
              s +
              "']"
          ).length
            ...
            let a = n.find(
              ...
              t = o.address_components.find((e) => "route" == e.types[0])
                ? o.address_components.find((e) => "route" == e.types[0])
                    .long_name
                : o.vicinity,
              r = e.returnAddressFRules(
                o.address_components,
                e.addressrules.state,
                "short_name"
              ),
              s = e.returnAddressFRules(
                o.address_components,
                "sublocality_level_1",
                "long_name"
              );
      ...
      sendAddress(e, o, a, n, t, r, s, d, m, h, C) {
        let i = this;
        ~C.indexOf(",")
          ? ((C = C.split(",")).forEach(function (e, o) {
              C[o] = parseFloat(e);
            }),
            "ARG" == e &&
              ("CABA" == r.toUpperCase() &&
      ...
      getRegions(e) {
        return t
          ...
          .regions.map(
            (e) => `<option value="${e.shortCode}">${e.name}</option>`
          );
      }
      getCountries() {
        let e = this;
        return e.deliveryCountries.map((o) => {
          let a = vtex.i18n[e.lang]
            ? vtex.i18n[e.lang]
          ...
          function (o) {
            if (
              ...
              let a = $(
                ...
                n = vtexjs.checkout.orderForm.shippingData;
              if (
